...
const SentencePractice = () => {
    ...
    const sentenceList = JSON.parse(localStorage.getItem("sentenceList") || "[]");
    ...
    useEffect(() => {
        setTargetText(sentenceList[sentenceIndex] || '');
    ...
    const handleSave = (sentenceData) => {
        ...
        fetch("http://localhost:5050/api/save-word", {
            ...
            .then(res => res.json())
            .then(() => alert(`"${sentenceData.word}" 저장되었습니다.`))
            .catch(err => {
    ...
    return (
        <div className="practice-wrapper">
            ...
            {!isFinished && !showResult && (
                <>
                    <h2 className="practice-title">{targetText ? `"${targetText}"` : '문장을 불러오는 중...'}</h2>
            ...
            {showResult && (
                <div className="dashboard-result">
                    <h2 className="dashboard-title">결과 요약</h2>
                    ...
                    <div className="bar-section">
                        <div className="score-bar">
                            <label>발음 정확도</label>
                            <div className="bar">
                                <div className="fill" style={{ width: `${pronunciation}%`, backgroundColor: '#3B82F6' }}></div>
                            </div>
                        ...
                        <div className="score-bar">
                            <label>입모양 점수</label>
                            <div className="bar">
                                <div className="fill" style={{ width: `${mouthShape}%`, backgroundColor: '#F97316' }}></div>
                            </div>
                            <span>{mouthShape}%</span>
                        </div>
                    </div>
                    ...
                    <div className="recognized-diff">
                        <h3>인식된 문장</h3>
                        <div className="diff-text">{getDiffColoredText()}</div>
                    </div>

                    <div className="dashboard-buttons">
                        <button className="retry-btn" onClick={handleRetry}>↺ 다시 녹음</button>
                        <button className="next-btn" onClick={handleNextSentence}>다음 문장</button>
                    </div>
                </div>
            )}
            ...
            {isFinished && (
                <div className="completion-fullscreen">
                    ...
                    <p>틀린 문장 {wrongSentences.length}개</p>
                    <div className="wrong-words-list">
                        {wrongSentences.map((item, i) => (
